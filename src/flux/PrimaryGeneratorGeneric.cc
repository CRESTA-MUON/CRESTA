#include "PrimaryGeneratorGeneric.hh"
#include "db/DB.hh"

namespace CRESTA {

//---------------------------------------------------------------------------------
PrimaryGeneratorGeneric::PrimaryGeneratorGeneric()
    : G4VUserPrimaryGeneratorAction(),
      fTime(0)
{
    std::cout << "FLX: Building Generic Source Generator" << std::endl;

    // Setup Table
    DBTable table = DB::Get()->GetTable("genericflux", "config");
    ...
    Analysis::Get()->SetFluxProcessor(new GenericPrimaryFluxProcessor(this));
...
void PrimaryGeneratorGeneric::GetSourceBox() {
...
void PrimaryGeneratorGeneric::GetTargetBoxes() {
...
bool PrimaryGeneratorGeneric::CheckTargetBoxes(G4ThreeVector& pos, G4ThreeVector& dir) {
...
void PrimaryGeneratorGeneric::SampleKinematics(G4ThreeVector & pos, G4ThreeVector & dir, G4double& energy) {
    ...
    // Throw direction isotropically
    G4double theta = fMinTheta + (fMaxTheta - fMinTheta) * G4UniformRand(); //theta uniform in [-pi, pi];
...
void PrimaryGeneratorGeneric::GeneratePrimaries(G4Event * anEvent) {
...
//------------------------------------------------------------------
GenericPrimaryFluxProcessor::GenericPrimaryFluxProcessor(PrimaryGeneratorGeneric* gen, bool autosave) :
    VFluxProcessor("shukla"), fSave(autosave)
{
    fGenerator = gen;
}

bool GenericPrimaryFluxProcessor::BeginOfRunAction(const G4Run* /*run*/) {

    std::string tableindex = "Generic";
    std::cout << "FLX: Registering GenericPrimaryFluxProcessor NTuples " << tableindex << std::endl;
...
bool GenericPrimaryFluxProcessor::ProcessEvent(const G4Event* /*event*/) {
...
G4double GenericPrimaryFluxProcessor::GetExposureTime() {
